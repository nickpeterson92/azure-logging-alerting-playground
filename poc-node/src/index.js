	...
	for (let i = 0; i < args.length; i++) {
		switch (args[i]) {
			...
			case "--delay":
				...
				break;
			case "--sql-failure-rate":
				options.sqlFailureRate = parseFloat(args[++i]);
				if (Number.isNaN(options.sqlFailureRate)) options.sqlFailureRate = 0.15;
				break;
			case "--sf-base-url":
				options.sfBaseUrl = args[++i] || "http://localhost:3001";
				break;
			default:
				break;
		}
...
async function main() {
	...
	console.log(`  Cycles: ${opts.cycles}  Delay: ${opts.delay}ms`);
	console.log(`  SQL failure rate: ${(opts.sqlFailureRate * 100).toFixed(0)}%`);
	console.log(`  Salesforce API:   ${opts.sfBaseUrl}`);
	console.log("==========================================================\n");
	...
	for (let cycle = 1; cycle <= opts.cycles; cycle++) {
		...
		let result;
		try {
			result = await integrationSim.runSyncCycle(cycle, {
				sqlFailureRate: opts.sqlFailureRate,
				sfBaseUrl: opts.sfBaseUrl,
			});
		} catch (err) {
		...
		for (let i = 0; i < result.events.length; i++) {
			...
			switch (evt.level) {
				case "info":
					...
					appinsights.trackSyncEvent("SyncInfo", {
						message: evt.message,
						...props,
					});
					break;
				...
				case "warning":
					...
					appinsights.trackSyncEvent("SyncWarning", {
						message: evt.message,
						...props,
					});
					appinsights.trackSyncException(new Error(evt.message), {
						severity: "warning",
						...props,
					});
					break;
				...
				case "error":
					...
					appinsights.trackSyncEvent("SyncError", {
						message: evt.message,
						...props,
					});
					appinsights.trackSyncException(new Error(evt.message), {
	...
	if (failureTypes.length > 0) {
		...
		for (let t = 0; t < failureTypes.length; t++) {
			console.log(
				`    ${failureTypes[t]}: ${metrics.failuresByType[failureTypes[t]]}`,
			);
		}
	}
	console.log("==========================================================\n");
	...
	appinsights.trackSyncMetric(
		"TotalRecordsFetched",
		metrics.totalRecordsFetched,
	);
	appinsights.trackSyncMetric(
		"TotalRecordsCreated",
		metrics.totalRecordsCreated,
	);
	appinsights.trackSyncMetric(
		"TotalRecordsUpdated",
		metrics.totalRecordsUpdated,
	);
	appinsights.trackSyncMetric(
		"TotalRecordsErrored",
		metrics.totalRecordsErrored,
	);
